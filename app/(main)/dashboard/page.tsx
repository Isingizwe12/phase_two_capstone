...
export default function DashboardPage() {
  ...
  // Fetch user's posts (both drafts and published)
  useEffect(() => {
    const fetchPosts = async () => {
      if (user) {
        try {
          setLoading(true);
          const userPosts = await getPostsByAuthor(user.uid, true); // true = include drafts
          setPosts(userPosts);
        } catch (error) {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-gray-600">Manage your posts and drafts</p>
          </div>
        </div>
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex-1">
            ...
            <Link href={`/post/${post.slug}`}>
              <h3 className="text-xl font-bold text-gray-900 hover:text-blue-600 mb-2 line-clamp-2">
                {post.title}
              </h3>
            </Link>
